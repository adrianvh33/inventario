...
function myFunction() {
    ...
    localStorage.setItem('toggleSaved',cam.checked);
 }

 function saveCheck(){
    const ubicacionSaved = localStorage.getItem('ubicacionSaved');
    let toggleSaved = localStorage.getItem('toggleSaved');
    if(ubicacionSaved){
...
function toggle(){
    ...
    if (cam.checked==true){
        document.getElementById("camara").style.display = 'inline';
        Quagga.init({
            inputStream : {
              name : "Live",
              type : "LiveStream",
          ...
          Quagga.onDetected(function(data){
            sonido.play()
            refe.value=data.codeResult.code;
            submitButton.click();
          });
    }else{
        Quagga.stop();
